...
import { AdminLayout } from "./Admin"; // reuse existing admin layout
...
function exportCSV(members: MemberRow[]) {
  const headers = [
    ...
    "How found", "Weekly email", "Interested in hosting",
    ...
    <>
      ...
      >
        ...
        <td className="px-5 py-4 text-sm text-muted-foreground">
          {member.country_from ?? member.nationality ?? "—"}
        </td>
        <td className="px-5 py-4 text-sm text-muted-foreground">
          {JOIN_LABELS[member.join_reason ?? ""] ?? "—"}
        </td>
        <td className="px-5 py-4 text-center">
          <span className="text-sm font-semibold text-foreground">{member.total_rsvps}</span>
        </td>
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {member.event_interests.map(i => (
                      <span key={i} className="text-xs px-2.5 py-1 bg-muted rounded-full text-foreground">
                        {i}
                      </span>
                    ))}
                    {member.event_interests_other && (
                      <span className="text-xs px-2.5 py-1 bg-muted rounded-full text-foreground italic">
                        {member.event_interests_other}
                      </span>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {member.looking_for.map(l => (
                      <span key={l} className="text-xs px-2.5 py-1 bg-muted rounded-full text-foreground">
                        {l.replace(/_/g, " ")}
                      </span>
                    ))}
                  </div>
...
export default function AdminMembers() {
  ...
  const filtered = members.filter(m => {
    ...
    const matchFilter =
      ...
      /* member */                !m.is_verified_organizer && !m.is_connector && !m.is_community_builder;
  ...
  const stats = [
    ...
    { label: "Interested in hosting", value: members.filter(m => m.interested_in_hosting).length },
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-border bg-popover text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
          />
        </div>
        <div className="flex gap-2 flex-wrap">
          {([
            ...
            { key: "wants_hosting", label: "Wants to host" },
          ] as const).map(f => (
            <button
              key={f.key}
